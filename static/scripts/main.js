...
$("#searcherbox").keyup(function(){
  ...
  if(last_search != text){
      last_search = text;
      $.get("https://onurb.me/api/searchout/autocomplete.php?q="+text.toLowerCase(), function(data){
...
function search(){
    ...
    $("#searcherbox").val("")
    $.get("https://onurb.me/api/searchout/search.php?s="+id+"&q="+text, function(data){  
        data = JSON.parse(data);
...
function sendChatMessage(){
    ...
    if(text.trim().length > 0){
        ...
        $.get("https://onurb.me/api/searchout/chat.php?q="+text+"&c="+complexity, function(data) {
            data = JSON.parse(data);
            let id = Date.now();
